...
# VERSÃO: 5.2 - ESTÁVEL (Compatível com Pydantic v1)
...
def create_task(db: Session, task_data: task_schemas.TaskCreateModel) -> Task:
    # --- CORREÇÃO CRÍTICA: Usando .dict() para Pydantic v1 ---
...
def get_all_tasks(db: Session, status: Optional[str], sort_by: str) -> List[Task]:
    ...
    if sort_by == 'priority':
        priority_order = case(
            (Task.priority == 'urgent', 0), (Task.priority == 'high', 1),
            (Task.priority == 'medium', 2), (Task.priority == 'low', 3),
            else_=4
        )
